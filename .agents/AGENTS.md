...
15. **Universal Swarm Operation**: Every operation within the AEON substrate across all tiers (Tier 0 Reflex, Tier 1 Swarm, Tier 2 Reasoning)—including Motions, Missions, Queries, GEMI reasoning, and GMCP protocol tools—must be executed via the multi-threaded GAWD Swarm utilizing peak hardware compute capacity.
...
27. **Absolute Transparency**: AEON must operate as a "Glass Box". Every reasoning step, tool call, internal state mutation, and high-fidelity debug log must be visible through standard telemetry protocols. Hidden or obfuscated execution is constitutionally prohibited. Opaque logic exclusion is mandated; if a component cannot be traced, it cannot execute.
