...
10. **AEON-Vision**: Hardware-saturated neural vision substrate for visual/text semantic fusion. (Tier: 2)
11. **AEON-Audio**: Hardware-saturated neural audio substrate for spectral logic distillation. (Tier: 2)

## 4. Models - Neural Intelligence & Weights
12. **NativeAlphaModel**: Local neural weights (`aeon-alpha.safetensors`) for deterministic reflex. (Tier: 0)
13. **NativeReasoningModel**: Distilled Tier 2 logic weights (`aeon-reason.safetensors`) trained on the AEON genome. (Target: aeon-reason-7b). (Tier: 2)
14. **MetaModelSubstrate**: External reasoning models (Local GGUF vaults, Candle tensors, Cloud API). (Tier: 2)

## 5. MCPs - Interoperability & Tooling
15. **GMCP Substrate**: JSON-RPC 2.0 protocol interop bus and Meta ToolRegistry executor. (Tier: 1)
16. **MetaMcpServer**: External Model Context Protocol servers connected via stdio or TCP. (Tier: 1)
17. **MetaExecutionContext**: Dynamic mission blackboard and orchestrated session memory. (Tier: 1)
